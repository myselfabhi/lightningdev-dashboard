		...
		<div className="proxy-details">
			...
			<div className="proxy-config">
				...
				<div className="config-row">
					...
					<input type="text" value="resi-www.lightningproxies.net" readOnly className="read-only-input" />
				</div>
				<div className="config-row">
					<label className="label">Port (HTTP & SOCKS5)</label>
					<input type="text" value="9999" readOnly className="read-only-input" />
				</div>

				{/* Rotating Proxy */}
				<div className="config-row">
					<label className="label">Rotating Proxy</label>
					<input type="text" value="resi-www.lightningproxies.net:9999:atjxdeqzgdlry100714-zone-resi:uepsjpxeji" readOnly className="read-only-input" />
				</div>

				{/* Sticky Sessions */}
				<div className="sticky-session-box">
					<div className="d-flex justify-content-between align-items-center">
						<label className="sticky-session-heading label">Sticky Sessions (Session time: {sessionTime} min)</label>
						<div className="toggle-wrapper">
							<label className="toggle-label me-2 label">Activate Super Sticky</label>
							<label className="switch label">
								<input type="checkbox" checked={isSuperSticky} onChange={handleSuperStickyToggle} />
								<span className="slider round"></span>
							</label>
						</div>
					</div>
					<input type="range" min={1} max={30} value={sessionTime} onChange={(e) => setSessionTime(Number(e.target.value))} className="session-slider" />
				</div>

				{/* Proxy Format Settings */}
				<div className="proxy-format-settings">
					<div className="d-flex justify-content-between align-items-center">
						<label className="proxy-format-heading label">Proxy Format Settings:</label>
						<input type="number" value={stickyCount} onChange={(e) => setStickyCount(Number(e.target.value))} min={1} max={1000} className="sticky-count-input" />
					</div>
				</div>
			</div>
			...
			<div className="proxy-output">
				<textarea rows={10} value={proxies.join('\n')} readOnly placeholder="Generated proxies will appear here textarea" />
				<div className="proxy-actions">
					<button className="button btn btn-primary" onClick={generateProxies}>
						Generate Proxies
					</button>
					<button className="button btn btn-outline-primary" onClick={copyToClipboard}>
						Copy Proxies
					</button>
					<button className="button btn btn-outline-secondary" onClick={downloadAsText}>
						Save as .txt
					</button>
				</div>
			</div>
