...
const InvoicesPage: React.FC = () => {
	...
	useEffect(() => {
		...
		if (search) {
			result = result.filter((invoice) => invoice.id.toLowerCase().includes(search.toLowerCase()))
		}
			...
			<div className="invoices-container">
				...
				<div className="controls bg-white p-3 d-flex justify-content-between align-items-center">
					<div className="search-bar d-flex align-items-center">
						...
						<input type="text" className="form-control" placeholder="Search your invoices" value={search} onChange={(e) => setSearch(e.target.value)} />
					</div>
					<div className="filter-buttons d-flex align-items-center">
						<span className="sort-label me-2">Sort by Payment:</span>
						<button className={`chip-btn chip-red me-2 ${filter === 'Unpaid' ? 'chip-active' : ''}`} onClick={() => setFilter('Unpaid')}>
							.Unpaid
						</button>
						<button className={`chip-btn chip-green ${filter === 'Paid' ? 'chip-active' : ''}`} onClick={() => setFilter('Paid')}>
							. Paid
						</button>
					</div>
				</div>
							...
							{filteredInvoices.length > 0 ? (
								filteredInvoices.map((invoice) => (
									<tr key={invoice.id}>
										...
										<td>{invoice.type}</td>
										<td>
											<span className={`chip ${invoice.paymentStatus === 'Paid' ? 'chip-success' : 'chip-danger'}`}>{invoice.paymentStatus}</span>
										</td>
