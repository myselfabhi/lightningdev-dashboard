...
const bandwidthPlans = [
	{
		title: 'Residential Day',
		price: '$300.00',
		details: [
			'2M+ Real Residential Peers',
			'IP & User:Pass Authentication',
			'Continent Targeting',
			'Rotating & Sticky Sessions',
			'Unlimited Bandwidth',
	...
	const renderCards = (plans: { title: string; price: string; details: string[] }[]) => {
		return plans.map((plan, index) => (
			<div key={index} className="col-12 col-sm-6 col-md-4 col-lg-3">
				<PlanCard title={plan.title} price={plan.price} details={plan.details} onPurchase={() => handlePurchase(plan.title)} />
			</div>
		))
	}
		...
		<DashboardLayout>
			<div className="purchase-plan-page">
				<div className="header d-flex justify-content-between align-items-center">
					<h4>Purchase Plan</h4>
					<div className="tabs">
						<button className={`btn ${activeTab === 'Residential' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setActiveTab('Residential')}>
							Residential
						</button>
						<button className={`btn ${activeTab === 'Bandwidth' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setActiveTab('Bandwidth')}>
							Bandwidth
						</button>
					</div>
				</div>
