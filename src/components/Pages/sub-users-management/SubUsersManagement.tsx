				...
				return (
					<div className="card p-4 shadow-sm">
						...
						<div>
							...
							<input type="text" className="form-control mb-3" placeholder="Enter customer name" value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
							<label>Proxy Username</label>
							<input type="text" className="form-control mb-3" placeholder="Enter proxy username" value={proxyUsername} onChange={(e) => setProxyUsername(e.target.value)} />
							<label>Proxy Password</label>
							<input type="password" className="form-control mb-3" placeholder="Enter proxy password" value={proxyPassword} onChange={(e) => setProxyPassword(e.target.value)} />
				...
				return (
					<div className="card p-4 shadow-sm">
						...
						<div>
							...
							<select className="form-control mb-3" value={selectedUsername} onChange={(e) => setSelectedUsername(e.target.value)}>
							...
							<input type="number" className="form-control mb-3" placeholder="Enter GB to add" value={gbToAdd} onChange={(e) => setGbToAdd(Number(e.target.value))} />
							<label>GB to Remove</label>
							<input type="number" className="form-control mb-3" placeholder="Enter GB to remove" value={gbToRemove} onChange={(e) => setGbToRemove(Number(e.target.value))} />
	...
	return (
		<DashboardLayout>
			<div className="container">
				...
				<div className="card p-4 shadow-sm">
					<div className="mb-3">
						<select className="form-select" onChange={(e) => setSelectedPlan(e.target.value)}>
					...
					<div className="accordion-container">
						<div className="tabs">
							<button className={`btn ${activeTab === 'Create' ? 'active' : ''}`} onClick={() => setActiveTab('Create')}>
								Create User
							</button>
							<button className={`btn ${activeTab === 'Delete' ? 'active' : ''}`} onClick={() => setActiveTab('Delete')}>
								Delete User
							</button>
							<button className={`btn ${activeTab === 'Manage' ? 'active' : ''}`} onClick={() => setActiveTab('Manage')}>
								Manage User
							</button>
						</div>
					</div>
