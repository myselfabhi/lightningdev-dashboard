...
import "./SubUsersManagement.module.css";
...
const SubUsersManagementPage: React.FC = () => {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      const response = await createUserResidential(
        proxyUsername,
        email,
        proxyPassword,
      );
  ...
  const handleAddGigabytes = async () => {
    if (!selectedUsername || gbToAdd <= 0) {
      toast.error(
        "Please select a username and provide a valid GB value to add.",
      );
      return;
    }
  ...
  const handleRemoveGigabytes = async () => {
    if (!selectedUsername || gbToRemove <= 0) {
      toast.error(
        "Please select a username and provide a valid GB value to remove.",
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        `Successfully removed ${gbToRemove} GB from ${selectedUsername}`,
      );
    } catch (error: unknown) {
      if (error instanceof Error) {
